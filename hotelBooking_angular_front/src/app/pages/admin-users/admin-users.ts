...
export class AdminUsersComponent implements OnInit, OnDestroy {
  ...
  loadUsers(): void {
    this.searchParams$.pipe(takeUntil(this.destroy$)).subscribe((params: any) => {
      this.store.dispatch(UsersActions.adminLoadUsers({ payload: params }));
    });
  }
  ...
  onCreateUser(): void {
    if (this.createUserForm.valid) {
      ...
      this.store.dispatch(UsersActions.adminCreateUser({ payload: userData }));
      
      this.createUserForm.reset();
      this.createUserForm.patchValue({ role: 'client' });
