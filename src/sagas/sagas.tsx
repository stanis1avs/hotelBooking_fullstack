...
import {
  deleteRoom, createRoom, updateRoom,
  deleteHotel, createHotel, updateHotel,
  recieveHotelPage, recieveSearchInfo } from '../reducers/typesHotelRoom'
import {
...
import {
  NewUser, NewClient, UserAuth,
  changeUserSearchParams
...
import {
  deleteReservation,
  CreateReservation,
  recieveResevationList } from '../reducers/typesReservationPage';
import {
...
import {
  historyRequest,
  socketConnectType, sendDataWSType
...
function* handleLogin(action: PayloadAction<UserAuth>): Generator<any, void, any> {
  try {
    const {id, email, name, contactPhone, role, access_token, refresh_token} = yield call(fetchRequestPost, "/api/auth/login", null, JSON.stringify(action.payload), {'Content-Type': 'application/json'});
...
function* handleRefresh(refreshToken: string): Generator<any, void, any> {
  try {
    const {access_token} = yield call(fetchRequestPost, "/api/auth/refresh", refreshToken);
    document.cookie = `access_token=${access_token};max-age=${2 * 24 * 60 * 60}`
  } catch (error: any) {
    yield put(authFailure(error.message));
  }
