import { Controller, Post, Get, Put, Delete, Body, Query, Param, Res, UseGuards, HttpStatus, UseInterceptors, UploadedFile, ParseFilePipe } from '@nestjs/common'
...
export class AdminHotelsController {
  ...
  async create(@Body() data: CreateHotel, @UploadedFile() file: Express.Multer.File, @Res() res: Response): Promise<void> {
    const createdHotel = await this.hotels.createHotel({...data, image: file.buffer});
  ...
  async editHotel(@Param('id') id, @Body() data: UpdateHotelParams, @UploadedFile() file: Express.Multer.File, @Res() res: Response): Promise<void> {
    const editedHotel = await this.hotels.editHotel(id, {...data, image: file ? file.buffer : data.image})
